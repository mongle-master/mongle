...
- 기록은 **UUID 소유자(ownerId)** 에 귀속되고 **소프트삭제**(`deletedAt`)를 상속한다.
- 필드: 제목(선택·40자)·메모(선택·200자)·날짜(필수)·시간(선택)·카테고리(필수 1)·날씨(선택 0~1)·감정(선택 ≤5)·연결 인물(필수 ≥1)·사진(선택 ≤5).
...
- `GET /api/v1/events/{id}` — 수정 모드 재사용용 상세: 연결 인물(id+이름)·칩(id+라벨, 감정/날씨/카테고리)·사진·메모·날짜·시간·(계산된)제목. 내 소유·active 만, 아니면 404 `NOT_FOUND`.
