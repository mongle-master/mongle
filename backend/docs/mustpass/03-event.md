...
- **언제 = 날짜(`YYYY-MM-DD`, 필수) + 시간(`HH:mm:ss`, nullable)**. 시간은 넣은 경우에만 의미.
- 칩·인물은 **id 참조**만 저장한다(라벨·이름 복사 안 함) — rename 하면 지난 기록에 저절로 반영(§12.2, [01-chip](01-chip.md)).

## 연결 인물 (must, #33)

- 기록↔인물 다대다는 **명시적 조인 모델**(`EventPerson`, id 참조·순서 보존)로 표현한다. 대표 인물 = **displayOrder 0**(첫 번째) 연결 인물.
...
- 저장 후 연결된 **각 인물**의 파생값을 갱신한다:
    - **마지막 만남:** 카테고리가 `만남`인 기록이면 각 인물의 마지막 만남을 기록 날짜로 전진 갱신한다. 더 최근일 때만 앞당긴다.
...
- **수정 파생 일관성 한계(의도적):** 수정으로 만남 날짜가 **더 과거로** 바뀌어도 인물의 `마지막 만남`을 **되돌리지 않는다**(저장 필드는 전진만). #30 파생 계산이 조회 시 이벤트 기준으로 다시 계산하므로 여기서 역계산까지 하지 않는다(과설계 회피).
...
- 단건은 `id + ownerId + deletedAt: null`로 소유·활성 상태를 함께 검증한다.
- 목록·타임라인은 `occurredDate desc → id desc` 순서를 유지한다.
- 홈·타임라인에서 여러 기록을 변환할 때는 `EventService.toResponses`와 `personIdsByEvent`의 배치 조회를 재사용해 기록별 N+1을 만들지 않는다.
