...
- `GET /api/v1/persons/{personId}/activity-flow` — 인물 소유·active 아니면 404.
...
- `GET /api/v1/timeline` — 내 전 인물 통합 연대기, 최신순.
...
| 상황 | 입력 | 기대 결과 |
|---|---|---|
| 없는·타인·삭제된 personId | GET /api/v1/persons/{id}/timeline | 404 `NOT_FOUND` |
| 연결 기록 없음 | GET /api/v1/persons/{id}/timeline | 200, 빈 리스트 |
...
| 전 기록 조회 | GET /api/v1/timeline | 최신순, 월 그룹, 카드에 연결 사람 |
| 카테고리+사람 동시 필터 | GET /api/v1/timeline?categoryChipIds=a&personIds=1 | (a) AND (인물1) |
| 다인 연결 카드 | GET /api/v1/timeline | persons 대표(즐겨찾기→가나다) 우선, 외 N명 |
