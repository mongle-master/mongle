  ...
  return (
    <>
      ...
      <div className="absolute top-1/2 left-1/2 z-20 flex -translate-x-1/2 -translate-y-[38px] flex-col items-center">
        ...
        <span
          data-amp-mask
          className="mt-1.5 max-w-24 truncate rounded-full bg-background/88 px-2 py-1 text-[12px] leading-none font-medium text-foreground shadow-e1 backdrop-blur-sm"
        >
          {me.name}
        </span>
      </div>
      ...
      {categories.map((category, index) => {
        ...
        return (
          <span
            ...
            data-amp-mask
            // text-zinc-700/200은 foreground와 muted-foreground 사이 값이라 전용 토큰 나오기 전까지 손튜닝 값으로 둔다
            className="absolute -translate-x-1/2 -translate-y-1/2 rounded-full bg-background/80 px-2 py-1 text-caption font-medium text-zinc-700 shadow-e2 dark:text-zinc-200"
            style={{
              left: `${position.x}%`,
  ...
  return (
    ...
    >
      ...
      {showText ? (
        <span
          data-amp-mask
          className="mt-1.5 flex max-w-full items-center gap-1 rounded-full bg-background/86 px-1.5 py-0.5 text-[12px] leading-none font-medium text-foreground shadow-e1 backdrop-blur-sm"
        >
