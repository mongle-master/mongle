...
function TimelinePhotoPreview({ photoUrls }: { photoUrls: string[] }) {
  ...
  return (
    <div className="relative size-20 shrink-0 overflow-hidden rounded-xl bg-muted shadow-inner">
      <img
        src={src}
        alt="기록 사진"
      ...
      />
      {photoUrls.length > 1 ? (
        <span className="absolute right-1 bottom-1 rounded-full bg-foreground/80 px-1.5 py-0.5 text-[10px] font-medium text-background">
          +{photoUrls.length - 1}
        </span>
      ) : null}
    </div>
