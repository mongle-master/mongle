...
function TimelinePhotoPreview({ photoUrls }: { photoUrls: string[] }) {
  ...
  return (
    <div className="relative size-20 shrink-0 overflow-hidden rounded-2xl bg-muted shadow-inner">
      ...
      />
      {photoUrls.length > 1 ? (
        <span className="absolute right-1 bottom-1 rounded-full bg-foreground/80 px-1.5 py-0.5 text-[10px] font-semibold text-background">
          +{photoUrls.length - 1}
        </span>
      ) : null}
    </div>
      ...
      <Card className="relative overflow-hidden py-0 shadow-e3 transition-all hover:-translate-y-0.5 hover:bg-muted/20 hover:shadow-e4">
        <CardContent className="p-4">
          <div className="flex items-start gap-2">
            <h3
              data-amp-mask
              className="min-w-0 flex-1 text-[17px] leading-snug font-semibold tracking-[-0.02em]"
            >
              {item.title}
            </h3>
            {item.category ? (
