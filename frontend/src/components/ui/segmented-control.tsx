    ...
    >
      {options.map((option) => (
        <ToggleGroupPrimitive.Item
          key={option.value}
          value={option.value}
          className="flex flex-1 items-center justify-center rounded-lg py-2.5 text-label font-medium text-muted-foreground transition-colors hover:text-foreground focus-visible:ring-[3px] focus-visible:ring-ring/50 focus-visible:outline-none"
        >
          {option.label}
        </ToggleGroupPrimitive.Item>
      ))}
      ...
      >
        {options.map((option) => (
          <span
            key={option.value}
            className="flex flex-1 items-center justify-center py-2.5 text-label font-medium"
          >
            {option.label}
          </span>
        ))}
