...
// badge.tsx가 이 문자열 위에 cva를 얹는다(부품 합성 선례). 높이는 시안 A의
// 칩 규격(32px)에 맞춘다. TagChip 자체는 아래 cva에서 높이·표면·채움색을
// 축으로 분해해 재조립한다.
export const tagChipBaseClass =
  'inline-flex h-8 w-fit shrink-0 items-center justify-center gap-1.5 rounded-full border border-transparent px-3.5 text-label leading-none font-medium whitespace-nowrap transition-colors'
...
const tagChipVariants = cva(
  'group/tag-chip inline-flex w-fit shrink-0 items-center justify-center gap-1.5 rounded-full border border-transparent leading-none whitespace-nowrap transition-colors',
  {
    variants: {
      ...
      // 함께 움직여 한 축으로 묶는다.
      size: {
        xs: 'h-5 px-2 text-[10px] font-medium',
        sm: 'h-6 px-2 text-caption font-medium',
        default: 'h-8 px-3.5 text-label font-medium',
        lg: 'h-9 px-3 text-label font-medium',
        xl: 'h-10 w-auto max-w-full shrink justify-start px-1.5 pr-3 text-label font-medium',
      },
