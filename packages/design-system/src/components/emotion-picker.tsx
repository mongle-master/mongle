...
function EmotionPicker({ value, onChange, className }: EmotionPickerProps) {
  return (
    <div data-slot="emotion-picker" role="radiogroup" aria-label="감정 선택" className={cn("flex gap-2", className)}>
      {(Object.keys(EMOTION_META) as Emotion[]).map((emotion) => (
        <button
          ...
          className={cn(
            "flex size-11 items-center justify-center rounded-full border border-border bg-secondary text-lg transition-all duration-150",
            "hover:border-muted-foreground/40 focus-visible:outline-2 focus-visible:outline-ring active:scale-95",
