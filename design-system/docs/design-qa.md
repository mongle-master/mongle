...
### 발견 3 — 감정 색은 "대기"만 있어 읽을 수가 없다

파스텔 orb 5색을 칩 글자에 그대로 쓰면 흰 카드 위 대비가 1.3~1.9:1이다 (전부
FAIL). 앱 채택 과정에서 감정 칩 텍스트를 토큰화하려다 드러났다.

- 조치: 프리미티브에 `--orb-*-strong` 5색을 추가하고 semantic에 텍스트 계층
  `--emotion-*-text`를 신설했다. 라이트=strong(전부 ≥4.5:1 — calm #187a63 5.25,
  warm #a06a3f 4.54, muse #755da0 5.48, clear #52739c 4.89, dear #a35f78 4.70),
  다크=파스텔(근검정 위 ≥10:1).
- design.md 원칙도 보강: "감정 색 추가는 5가족 안에서 명도만 조절" → 대기/읽기
  2계층 모두 이 규칙을 따른다.
